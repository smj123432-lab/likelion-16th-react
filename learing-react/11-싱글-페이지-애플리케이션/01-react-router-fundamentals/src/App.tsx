        ...
        <BrowserRouter>
          <Routes>
            ...
            <Route path={NAVIGATION_PATH.base} element={<CommonLayout />}>
              {/* 네비게이션 패스의 베이스로 넘어가면 컴포넌트 레이아웃을 띄운다 주소창도 바뀜 */}
              {/* 중첩된, 인덱스 라우트 */}
              <Route index element={<Home />} />
              {/* 인덱스 엘리먼트 -> 도메인으로 들어갔을 떄 보여줄 컴포넌트 */}
              {/* 경로(path)를 가지는 라우트 */}
              <Route path={NAVIGATION_PATH.login} element={<Login />} />
              {/* 로그인으로 넘어가면 로그인 페이지를 띄운다 */}
              {/* 다이내믹 세그먼트 라우트 */}
              <Route
                path={`${NAVIGATION_PATH.movies}/:movieId`}
                // 이걸 작성 후 해당하는 컴포넌트 안에서 유즈파람스를 이용하면 movieId가 해당하는 영화의 아이디 값으로 바뀜
                element={<MovieDetail />}
                // 무비스로 넘어가면 무비디테일로 간다
              />
              {/* 프로텍티드 라우트 */}
              <Route
                ...
                element={
                  ...
                  // 조건을 걸어서 조건에 통과하면 보여주도록 보안을 신경 쓴 구조
                }
              />
              {/* 보호된 라우트 예시 (예: 배우들) */}

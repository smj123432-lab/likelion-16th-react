...
export function ThemeProvider({ children }: React.PropsWithChildren) {
  ...
  const [theme, setThemeState] = useState<Theme>(() => {
    ...
    // 테마키가 맞으면 세이브를 테마의 시스템 타입에 맞게 객체로 생성
  });

  /* 시스템 테마 실시간 구독 (React 18+ 최적화) */
  // useEffect 없이도 외부 시스템의 변화에 따라 컴포넌트를 재렌더링합니다.
  const isSystemDark = useSyncExternalStore(
    // useEffect는 화면이 그려진 후에 실행되지만,
    // 얘는 화면을 그리는 도중에도 브라우저 설정(다크모드 여부)이 바뀌면
    // 즉시 감지해서 리액트에게 "어이, 지금 당장 다시 그려!"라고 신호를 줍니다.

    // 섭스크라이브시스템테마와 겟스타일스냅샷이 바뀌면 리렌더링?

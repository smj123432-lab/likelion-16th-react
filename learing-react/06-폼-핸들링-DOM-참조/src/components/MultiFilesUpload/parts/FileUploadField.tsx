...
interface Props {
  ...
  // Preview 타입에서 id, url의 타입을 직접 꺼내서 사용
  // Preview['id'] = string, Preview['url'] = string 과 동일
  // 타입을 직접 쓰지 않고 Preview에서 참조 → Preview가 바뀌면 자동 반영
}
...
export default function FileUploadField({
  ...
  // Props로 받은 데이터를 활용해서 아래 JSX(마크업)를 렌더링한다
  // Props 타입으로 정의된 객체를 구조분해할당으로 꺼냄
  // ref: input DOM 연결용
  // previews: 미리보기 목록 표시용
  // onFileChange: 파일 선택 시 부모 함수 실행용
  // onDeleteFile: 미리보기 삭제 시 부모 함수 실행용
